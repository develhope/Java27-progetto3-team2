...
public class CartItemMapper {
    ...
    public CartItemDTO cartItemToCartItemDTO(CartItem cartItem){
        return CartItemDTO.builder().id(cartItem.getId()).menuItemId(cartItem.getMenuItem().getId()).quantity(cartItem.getQuantity()).cartId(cartItem.getCart().getId()).build();
    }
    public CartItem cartItemDTOToCartItem(CartItemDTO cartItemDTO){
        return CartItem.builder().id(cartItemDTO.getId()).menuItem(menuItemService.getMenuItemById(cartItemDTO.getMenuItemId())).quantity(cartItemDTO.getQuantity()).cart(cartService.getCartById(cartItemDTO.getCartId())).build();
    }
